...
for sheet_name, df in sheets.items():
    print('Sheet', sheet_name, 'rows', len(df.columns), 'cols', len(df))
    if df.shape[0] == 0:
        continue
    ...
    for i,row in enumerate(df.itertuples(index=False, name=None)):
        if i >= 3:
            break
        rv = list(row[:len(DB_COLUMNS)])
        if len(rv) < len(DB_COLUMNS):
            rv.extend([None] * (len(DB_COLUMNS)-len(rv)))
        year_val = rv[year_index]
        print('row', i+1, 'year', year_val, 'fill?', year_val if year_val else '2022-2023', 'len(rv)', len(rv))
